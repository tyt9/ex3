...
class Ex3ApplicationTests {
    ...
    @Transactional
    @Test
    public void 엔티티매니저_연습(){ //<< entity manager 단독 실행 / jpaRepository 없이
        MyData myData = em.find(MyData.class, 1L);
        System.out.println(myData);
        myData.setName("김하나");  //<< 업데이트 없이 값이 바뀜
        myData = em.find(MyData.class, 1L);
        System.out.println("get"+myData.getName());
        System.out.println(myData);
    }
    ...
    @Test
    public void modify_myData(){
        myDataRepository.updateMyData(2L,"modify_MyData");
    ...
    public void update_MyData(){
        MyData myData = MyData.builder().id(3L).memo("수정").build();
